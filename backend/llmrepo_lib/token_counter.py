...
def count_message_tokens(messages, model="gpt-3.5-turbo-0613"):  # noqa: PLR0912,C901
...
def count_string_tokens(string: str, model_name: str = "gpt-4") -> int:
    """
...
def process_input(input_text, model):
    token_count = count_string_tokens(input_text, model)
    logger.info(f"Token count for model {model}: {token_count}")
    if model in TOKEN_COSTS:
        prompt_cost = token_count * TOKEN_COSTS[model]["prompt"] / 1000
        logger.info(f"Estimated prompt cost: ${prompt_cost:.6f}")


def main():
    parser = argparse.ArgumentParser(
        description="Count tokens from text input or files.",
    )
    ...
    if not args.files:
        ...
        process_input(input_text, args.model)
    else:
        # Process each file
        for file_path in args.files:
            try:
                ...
                logger.info(f"Error reading file {file_path}: {e}", file=sys.stderr)
