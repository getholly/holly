...
containers=$(docker ps --format "{{.ID}} {{.Names}}" | awk '$2 ~ /^pr-/ {print $1}')

# Stop matching containers
for container in $containers; do
  echo "Stopping container named 'pr-*': $container"
  docker ps -a | grep $container
  docker stop "$container"
done

# Find all containers (running or stopped) with names starting with 'holly-container'
all_matching=$(docker ps -a --format "{{.ID}} {{.Names}}" | awk '$2 ~ /^pr-/ {print $1}')
for container in $all_matching; do
  echo "Removing container named 'pr-*': $container"
  docker rm "$container"
done
