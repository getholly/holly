...
pub fn render_llms(f: &mut Frame, app: &App) {
    ...
    if app.llm.llms.is_empty() {
        ...
        let items: Vec<ListItem> = app.llm.llms.iter().enumerate().map(|(i, l)| {
            let provider = l.provider.as_deref().unwrap_or("—");
            let text = format!("{:<30} [{}]", l.name, provider);
            let style = if i == app.llm.selected_idx { selected_style() } else { normal_style() };
            ListItem::new(text).style(style)
        }).collect();
        let mut state = ListState::default();
        state.select(Some(app.llm.selected_idx));
    ...
    if app.llm.api_keys.is_empty() {
        ...
        let items: Vec<ListItem> = app.llm.api_keys.iter().map(|k| {
